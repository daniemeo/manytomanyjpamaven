...
public class UtenteDAOImpl implements UtenteDAO {
	...
	public void insert(Utente utenteInstance) throws Exception {
		if (utenteInstance == null || esisteUsername(utenteInstance)) {
			throw new Exception("Problema valore in input");
		}
	...
	@Override
	public boolean esisteUsername(Utente utente) throws Exception {
		 List<Utente> listaUtenti = new ArrayList<>();
		 listaUtenti = list();
		 
		 for (Utente utente1 : listaUtenti) {
			 if(utente1.getUsername().compareTo(utente.getUsername())==0) {
				 return true;
			 }
		 }
		 return false;
